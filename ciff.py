    ...
    def parse_ciff_file(file_path):
        ...
        try:
            with open(file_path, "rb") as ciff_file:
                ...
                if len(magic) != 4:
                    raise Exception("IncorrectMagicSize")
                bytes_read += 4
                ...
                if len(h_size) != 8:
                    raise Exception("IncorrectHeaderSize")
                bytes_read += 8
                ...
                if (new_ciff.header_size < 38 or new_ciff.header_size > pow(2, 64) -1):
                    raise Exception("IncorrectHeader")

                # read the content size
                c_size = ciff_file.read(8)
                # TODO: c_size must contain 8 bytes. If not, raise Exception
                if len(c_size) != 8:
                    raise Exception("IncorrectContentSize")
                bytes_read += 8
                # interpret the bytes as an 8-byte-long integer
                ...
                if new_ciff.content_size < 0 or \
                        new_ciff.content_size > pow(2, 64) -1:
                    raise Exception("IncorrectContent")
                ...
                # TODO: check the value range. If not in range, raise Exception
                # Question: is this check necessary?
                if new_ciff.width < 0 or \
                        new_ciff.width > pow(2, 64) -1:
                    raise Exception("IncorrectWidth")
                ...
                # Question: is this check necessary?
                if new_ciff.height < 0 or \
                        new_ciff.height > pow(2, 64) -1:
                ...
                while bytes_read != new_ciff.header_size:
                    ...
                    if (bytes_read == new_ciff.header_size -1) and c == '\0':
                        tags.append(tag)
